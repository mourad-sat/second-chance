...
export function PublicRegistrationForm() {
  ...
  return <form ref={formRef} onSubmit={submit} className="space-y-5" onChange={() => { if (draftLoaded) saveDraft(); }}>
    ...
    {step === 2 && <section className={section}><h2 className="mb-6 text-xl font-black">المسار الدراسي</h2><div className="grid gap-5 md:grid-cols-2">
      <label className="text-sm font-bold">آخر مستوى دراسي *<select required name="lastEducationLevel" className={input} defaultValue=""><option value="">اختر</option><option>ابتدائي</option><option>إعدادي</option><option>ثانوي تأهيلي</option><option>تكوين مهني</option><option>غير متمدرس</option></select></label>
      ...
      <label className="text-sm font-bold">استفدت من برنامج مشابه؟<select name="previousProgram" className={input} defaultValue=""><option value="">اختر</option><option>لا</option><option>نعم</option></select></label>
      <label className="text-sm font-bold md:col-span-2">سبب الانقطاع *<textarea required name="dropoutReasons" rows={3} className={input} /></label>
      <label className="text-sm font-bold md:col-span-2">الصعوبات الدراسية<textarea name="learningDifficulties" rows={3} className={input} /></label>
    </div></section>}

    {step === 3 && <section className={section}><h2 className="mb-6 text-xl font-black">الرغبات والأهداف</h2><div className="grid gap-5">
      <label className="text-sm font-bold">المشروع الشخصي *<textarea required name="personalProject" rows={4} className={input} /></label>
