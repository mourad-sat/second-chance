...
import { CheckCircle2, Circle, Clock3, Loader2, Search, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={lookup} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <button disabled={loading} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-blue-800 px-5 py-3 font-black text-white hover:bg-blue-900 disabled:opacity-50">
      ...
      {result && meta && (
        <section className="overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-xl shadow-slate-200/50">
          <div className="bg-slate-950 px-6 py-6 text-white sm:px-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-slate-300">المترشح</p>
                <h2 className="mt-1 text-2xl font-black">{result.fullName}</h2>
                <p className="mt-2 font-mono text-sm text-slate-300">{result.registrationNumber}</p>
              </div>
              <div className={`rounded-full px-5 py-3 text-sm font-black ${meta.tone}`}>{meta.label}</div>
            </div>
          </div>

          <div className="p-6 sm:p-8">
            <div className="rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm leading-7 text-blue-950">
              {meta.message}
            </div>

            <div className="mt-8 grid gap-4 sm:grid-cols-5">
              {steps.map((step, index) => {
                const complete = index + 1 <= meta.step;
                return (
                  <div key={step} className="flex items-center gap-3 sm:flex-col sm:text-center">
                    {complete ? <CheckCircle2 className="shrink-0 text-emerald-600" size={26} /> : <Circle className="shrink-0 text-slate-300" size={26} />}
            ...
            <div className="mt-6 flex items-start gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-4 text-sm leading-7 text-emerald-950">
              <ShieldCheck className="mt-1 shrink-0 text-emerald-700" size={20} />
              <p>هذه الصفحة مخصصة للاستعلام فقط. ستتواصل إدارة البرنامج معكم عبر رقم الهاتف المسجل عند الحاجة إلى إجراء إضافي.</p>
            </div>
          </div>
        </section>
      )}
